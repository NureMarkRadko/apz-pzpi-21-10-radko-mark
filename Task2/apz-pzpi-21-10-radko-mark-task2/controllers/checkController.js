...
class checkController {
    ...
    async getWhereUser(req, res) {
        try {
            ...
            console.error('Ошибка при получении чека пользователя:', error);
            return res.status(500).json({ message: 'Ошибка при получении чека пользователя' });
        }
    }
    ...
    async getWhereDepartment(req, res) {
        try {
            ...
            if (!checks || checks.length === 0) {
                return res.status(404).json({ message: 'Чеки для отдела не найдены' });
            }
            ...
            console.error('Ошибка при получении чеков отдела:', error);
            return res.status(500).json({ message: 'Ошибка при получении чеков отдела' });
        }
    }
